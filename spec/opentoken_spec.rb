...
describe OpenToken do
  ...
  describe ".decode" do
    ...
    let(:otk) { OpenToken::OTK.new(password: 'Test123') }
      ...
      context "when current time is outside clock skew tolerance before expiration date" do
        it do
          Timecop.freeze(Time.iso8601('2010-03-04T19:19:05Z')) do
            expect { otk.decode opentoken }.to raise_error OpenToken::TokenExpiredError
          end
        end
      end
      context "when current time is within clock skew tolerance before expiration date" do
        it do
          Timecop.freeze(Time.iso8601('2010-03-04T19:19:11Z')) do
            expect { otk.decode opentoken }.to_not raise_error
          end
        end
      end
    end
    ...
    context "with a nil token" do
      it { expect { otk.decode(nil) }.to raise_error OpenToken::TokenInvalidError }
    end

  end

  describe ".encode" do
    let(:otk) { OpenToken::OTK.new(password: 'Password1') }

    context "with aes-128-cbc and subject attribute" do
      let(:attributes_in) { { "subject" => "john", "email" => "john@example.com" } }
      let(:token) { otk.encode attributes_in, OpenToken::Cipher::AES_128_CBC }
      it { otk.decode(token).should == attributes_in }
    end
